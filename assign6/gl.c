...
void gl_draw_pixel(int x, int y, color_t c) {
    ...
    if (y >= fb_get_height() || x >= per_row) return;
    if (y < 0 || x < 0) return;
    im[y][x] = c;
}
...
color_t gl_read_pixel(int x, int y) {
    ...
    if (y >= fb_get_height() || x >= per_row) return 0;
    if (y < 0 || x < 0) return 0;
...
void gl_draw_rect(int x, int y, int w, int h, color_t c) {
    ...
    int x_max = (x + w) <= per_row ? (x + w) : per_row; 
    ...
    for (int loc_x = x_min; loc_x < x_max; loc_x++) {
        for (int loc_y = y_min; loc_y < y_max; loc_y++) {
            im[loc_y][loc_x] = c;
        }
    }
}
...
void gl_draw_char(int x, int y, char ch, color_t c) {
    ...
    // doesn't do anything if ch is not a valid char in font
